...
## 资源组隔离
在 TiDB HTAP 下 TiFlash 会有以下场景:
- TP 加速引擎，加速轻量级的 TP 查询，消耗 cpu 少，执行时间短  
- AP 实时分析，执行偏重量级的 AP 查询，消耗 cpu 多，执行时间长 
 
两者需要做到资源隔离，需要保证后者不影响前者的快速执行。  
在 TiFlash 原先的执行模型下，要做好资源隔离只能划分集群，TP 加速和 AP 实时分析使用不同的集群。这中间就存在了机器资源浪费的问题。  
在 pipeline model 下，我们可以对 task 调度策略做定制化设定，对 TP 加速和 AP 实时分析在同一个集群里划分不同的资源组。
- TP 加速引擎资源组优先级高，保证 TP 查询能够快速执行
- AP 实时分析资源组优先级低，保证 AP 查询在不影响 TP 查询下平稳执行，不会被饿死

并且两个资源组都能充分利用集群的所有 cpu，避免了资源浪费的问题。
## 多租户
多租户是降低客户上云成本的一个重要手段，让多个租户共享一个大集群来均摊单个租户的成本。而多租户的一个关键就是需要做到多租户之间的隔离。  
在 TiFlash 原先的执行模型下计算层是无法做资源隔离的，也就不具备多租户的基础。  
但是在 pipeline model 下，我们可以对 task 调度策略做定制化设定，对租户划分独立的资源组，保障多租户之间的隔离。  
多租户不仅仅可以让客户降低成本，甚至可以在一定程度上让客户享受到更快的速度。例如东半球的用户和西半球的用户共用一个集群，因为东西半球的时差，在各自的工作时间段里，他们可以独享整个集群。  
