...
## 多租户
多租户是降低客户上云成本的一个重要手段，让多个租户共享一个大集群来均摊单个租户的成本。  
而多租户的一个关键就是需要做到多租户之间的隔离。  
在 TiFlash 原先的执行模型下计算层是无法做资源隔离的，也就不具备多租户的基础。  
但是在 pipeline model 下，我们可以对 task 调度策略做定制化设定，对租户划分独立的资源组，保障多租户之间的隔离。  
多租户不仅仅可以让客户降低成本，甚至可以在一定程度上让客户享受到更快的速度。  
例如东半球的用户和西半球的用户共用一个集群，因为东西半球的时差，在各自的工作时间段里，他们可以独享整个集群。  
